...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        {
            // remove the oldest element at the beginning of the buffer
        ...
        if (bFocusOnVehicle)
        {
            std::vector<cv::KeyPoint> filtKeypoints;

            for (int i = 0; i < keypoints.size(); i++)
            {
                cv::Point2i kpt = static_cast<cv::Point2i>(keypoints[i].pt);
                if (vehicleRect.contains(kpt))
                {
                    filtKeypoints.push_back(keypoints[i]);
                }
            }

            keypoints.clear();
            keypoints = filtKeypoints;
        }
        ...
        spreadsheet << "Detection Time: " << detTime << "\n";
		spreadsheet << "Detected Keypoints: " << NDetected << "\n";
        spreadsheet << "Description Time: " << desTime << "\n";
		spreadsheet << "Matched Keypoints: " << NMatched << "\n\n";
